...
;;; Start the server
(define (go)
  (define log-to (string-append (getenv "HOMEPATH")
                                "\\racket-server-access.txt"))
  (define (kickoff request) 
    (define a-vat (new vat% (request request)))
    (vat-page a-vat request))
  (serve/servlet kickoff #:port 80 #:listen-ip #f #:log-file log-to))
